<script lang="ts">
  ...
  async function hydrate() {
    errorMsg = undefined;
    try {
      [marketplaceCurrency] = await Promise.all([getMarketplaceCurrency()]);
    ...
    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-semibold">
          Modify Listing #{String(listing.id)}
        </h3>
      ...
      <div class="space-y-4">
        <div>
          <label for="modifyPriceUsd" class="block text-sm text-gray-600 mb-1"
            >Price (USD)</label
          >
          <input
            id="modifyPriceUsd"
        ...
        <div>
          <label for="modifyQuantity" class="block text-sm text-gray-600 mb-1"
            >Quantity</label
          >
          <input
            id="modifyQuantity"
