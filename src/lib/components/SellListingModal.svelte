<script lang="ts">
  ...
  function parseBigintFromDecimalString(
    value: string,
    decimals: number
  ): bigint | undefined {
...
{#if open}
  ...
  >
    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
      ...
      <div class="space-y-4">
        <div>
          <label for="availableToSell" class="block text-sm text-gray-600 mb-1"
            >Available to Sell</label
          >
          <div id="availableToSell" class="text-gray-900">
            {available !== undefined ? String(available) : '—'}
          </div>
        </div>

        <div>
          <label for="sellQuantity" class="block text-sm text-gray-600 mb-1"
            >Quantity</label
          >
          <input
            id="sellQuantity"
        ...
        <div>
          <label for="sellPriceUsd" class="block text-sm text-gray-600 mb-1"
            >Price (USD)</label
          >
          <input
            id="sellPriceUsd"
            ...
            step="0.01"
            min="0"
            bind:value={priceUsdStr}
            placeholder="0.00"
          />
          {#if floorPriceCents}
            <p class="mt-1 text-xs text-gray-500">
              Floor: ${(Number(floorPriceCents) / 100).toFixed(2)} USD
            </p>
          {/if}
        </div>
