...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  ...
  {#if sections.length === 0}
    <div
      class="rounded-xl border border-gray-200 bg-white p-6 text-center text-gray-600"
    >
      No tokenized businesses available.
    </div>
  {:else}
    {#each sections as s (String(s.releaseId))}
      <div
        class="bg-white rounded-xl border border-gray-200 overflow-hidden mb-8"
      >
        <div
        ...
        >
          ...
          <div>
            <Button variant="outline" href={`/business/${s.business.id}`}
              >View</Button
            >
          </div>
        </div>
                ...
                {#each s.listings as l (String(l.id))}
                  <tr class="hover:bg-gray-50">
                    <td
                      ...
                      >{formatUsdCents(l.pricePerItem)}</td
                    >
                    <td
                      class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"
                      >{String(l.quantity)}</td
                    >
                    <td
                      class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"
                      >{shortAddress(l.seller)}</td
                    >
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                      <div class="flex space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          href={`/business/${s.business.id}`}>View</Button
                        >
                ...
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <div class="font-medium text-gray-900">
                      {formatUsdCents(l.pricePerItem)}
                    </div>
                    <div class="text-sm text-gray-500">
                      {String(l.quantity)} shares • Seller {shortAddress(
                        l.seller
                      )}
                    </div>
                  </div>
